...
## 一、介绍与讲解（`docs/introduction/`）

| 文档 | 用途 | 时效 |
|---|---|---|
| [面试讲解手册.md](introduction/面试讲解手册.md) | **主文档**。话术、10 个创新点、**11 个踩坑**（坑 10 能力题出砸 + C-8 已毕业、**坑 11 `[WORK]` 对照双重无效，G1′ k=1 开过关不过**）、边界划分、数字速记 | 🟢 |
| [保研面试PPT大纲.md](introduction/保研面试PPT大纲.md) | PPT 逐页内容 + 给 PPT AI 的指令。**只管"放什么"，话术在手册里** | 🟢 |
| [主循环流程图-面试讲解.md](introduction/主循环流程图-面试讲解.md) | 单图深讲：ReAct 骨架、三重闸门、子代理怎么开 | 🟢 |
| `introduction/主循环流程图.drawio` | 上一篇的矢量图源文件 | 🟢 |
| [Agent评测体系使用指南.md](introduction/Agent评测体系使用指南.md) | 怎么跑评测、怎么录制、**怎么判断报告可信**、成本预算门禁与能力集、上下文工程基准 | 🟢 |
...
| [Agent量化评测-P0至P5实施设计.md](design/Agent量化评测-P0至P5实施设计.md) | **怎么做**：**P0~P5 全部落地**（含实施中改过的决定）。P1 真实录制已完成（13×3，§3.7 分诊）；**P5 推翻了自己的初稿**——token 门禁在回放下结构上永远绿，改建在活体字符数上（§7.1~7.2） | 🟢 |
| [Agent量化评测-P6对标腾讯评测指南.md](design/Agent量化评测-P6对标腾讯评测指南.md) | **对标企业实践**：Anthropic 8 要素逐条比对。⚠ 多轮对照当时结论不成立；后来发现窗口配置也没接到 `ConversationMemory`，实验双重无效 | 🟢 |
| [Agent优化-P7窗口接线与证据空白.md](design/Agent优化-P7窗口接线与证据空白.md) | **P7**（2026-09-14）：窗口已接线；C-8 3/3 毕业；G1′ k=1 开过关不过；Facts P=1.0/R=0.75 仍关 | 🟢 |
...
| [知识目的地-规划与知识双入口IA.md](design/知识目的地-规划与知识双入口IA.md) | 应用顶栏拆成规划 / 知识；知识是工作室，检验进工具 | 🟢 |
...
| [学习体系收敛-纪律层未合上.md](problems/学习体系收敛-纪律层未合上.md) | **P-KB-002**。G1～G12 收口后仍停在纪律层的五项；第 1 项（接入即 Git 检索）代码已合上 | 🟢 |
