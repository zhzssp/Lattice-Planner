...
| [Agent量化评测-P0至P5实施设计.md](design/Agent量化评测-P0至P5实施设计.md) | **怎么做**：P0~P4 全部落地（含实施中改过的决定）。**P1 的真实录制已完成**（13×3，见 §3.7 分诊），P5 待做 | 🟢 |
| [功能验收方案.md](功能验收方案.md) | 各能力的手工验收步骤与断言。**P11 档验收「量表本身准不准」**。附 **C-5**（指标算出来的三条缺陷）与 **C-6**（换真实录制暴露的三条），**六条均已修复**，含效果数字 | 🟢 |
