...
| 文档 | 用途 | 时效 |
|---|---|---|
| [面试讲解手册.md](面试讲解手册.md) | **主文档**。话术、10 个创新点、**11 个踩坑**（坑 8 真实录制换血、坑 9 推翻自己的成本门禁设计、坑 10 我出的能力题被 3/3 做完、**坑 11 精心设计的对照实验得出否定结论**）、边界划分、数字速记 | 🟢 |
...
| [Agent量化评测-P6对标腾讯评测指南.md](design/Agent量化评测-P6对标腾讯评测指南.md) | **对标企业实践**：Anthropic 8 要素逐条比对（全部具备，判官校准与成本门禁两处更进）。补齐**多轮**与**安全合规**两个空白。⚠ **多轮对照实验得出否定结论**——模型会自我复述约束，折叠开/关都 3/3，故**尚无端到端证据支撑滚动摘要收益**（§4） | 🟢 |
| [功能验收方案.md](功能验收方案.md) | 各能力的手工验收步骤与断言。**P11 档验收「量表本身准不准」**。附 **C-5**（指标算出来的三条）、**C-6**（换真实录制暴露的三条）、**C-7**（P5 期间两条量具事故）、**C-8**（能力集查出、**有意留红不修**的确认策略不一致） | 🟢 |
