...
Lattice-Planner 是一款面向个人的规划与任务管理软件，由**服务端 Web 应用**与可选的 **Windows 桌面客户端（Electron）**组成。用户通过浏览器或桌面客户端访问服务端，完成注册与登录后，可在不同思维模式（执行 / 学习 / 规划）下，对任务、目标、笔记进行管理，并查看规划得分曲线与 AI 总结。桌面客户端在用户登录状态下可常驻系统托盘，定期检查任务截止时间并发送桌面通知提醒。
...
- **版本号**：V1.0  
- **开发完成日期**：【请按实际填写，如：2025 年 XX 月 XX 日】  
- **首次发表日期**：【请按实际填写；未发表可填“未发表”】  
- **著作权人**：【按申请人/单位实际信息填写】  
...
- **Java 运行环境**：JDK 17 及以上（实际项目使用 JDK 21）。  
...
### 3.4 开发环境与工具（软著“开发环境/工具”参考）

- 操作系统：Windows 10 64 位 PC（x86_64）。  
- 开发工具：IntelliJ IDEA / VS Code 等。  
- 编程语言与运行环境：Java 21、HTML/CSS/JavaScript。  
...
本章整合了原《用户手册》中的功能与操作说明，并结合设计文档中的相关描述，去掉重复表述。
...
## 第 9 章 源程序与文档鉴别材料说明（软著相关）

### 9.1 源程序鉴别材料

- 要求提交**源程序前连续 30 页与后连续 30 页**，共 60 页，每页不少于 50 行（最后一页可少于 50 行）。  
- 建议仅提交本软件原创源代码（Java、HTML、CSS、JavaScript、Electron 主进程与预加载脚本等），不包含第三方库源代码。  
- 本项目已提供脚本用于生成前/后 30 页的源程序 Markdown：  
  - 脚本：`docs/generate_source_snippets.py`。  
  - 输出：`docs/source_program_front_back.md`，可用 Typora / VS Code / Word 导出为 PDF。  

### 9.2 文档鉴别材料

- 可选择本综合文档（或原《软件设计说明书》或《用户手册》）作为文档鉴别材料来源。  
- 按登记要求截取**前 30 页 + 后 30 页**（共 60 页），每页不少于 30 行（最后一页除外）。  
- 建议将本综合文档或设计说明书转换为 Word/PDF 后，再进行分页与打印/导出。  

---

## 第 10 章 版本与维护

- 本综合文档对应 Lattice-Planner V1.0 当前实现，已整合并替代原分散的多份说明文档（软件简要说明、软件设计说明书、用户手册、架构说明等）中的重复内容。  
- 后续版本如有架构、接口或功能变更，应同步更新本综合文档与实际实现，并在文首更新文档版本与修订日期。  
