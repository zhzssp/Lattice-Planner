...
- **UserPreference**：存储每用户偏好。除 maxVisibleTasks、showFutureTasks、showStatistics 外，还包括：theme（背景亮/暗）、defaultMindsetMode（默认思维模式）、defaultTaskView（规划模式默认视图）、fuzzyTaskDaysThreshold（模糊任务提示天数）、showGoals、showFuzzyHint、showArchivedSection、showScoreSection 等；偏好设置页按思维模式分组展示，仅显示当前所选模式下的可配置项。  
- **思维模式**：通过 FeatureSelectionController 与 Session 维护当前模式（execute/learn/plan），规划模式下可访问洞察相关页面；是否显示统计入口还受 showStatistics 控制。  
- **目标与任务树**：看板右上角提供「目标与任务树」按钮，默认隐藏；点击后展示以节点与边构成的树状图，仅含有关联关系的目标及其下属任务，三种模式均可查看；前端使用 D3 绘制，数据由 GET /api/goal-task-tree 提供。
...
- GET /api/goal-task-tree：返回当前用户「目标–任务」层级结构（JSON），供看板树状图（节点与边）渲染使用；需认证。
