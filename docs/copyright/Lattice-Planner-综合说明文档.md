...
- **用户名输入框**：输入已注册的用户名（必填）。
- **密码输入框**：输入对应的密码（必填）。
- **Login 按钮**：提交登录表单，验证用户名和密码后跳转到功能选择页面。若用户名或密码错误，会显示错误提示信息。
- **Register 按钮**：跳转到注册页面。

#### 9.1.2 注册页面操作

- **用户名输入框**：输入唯一的用户名（必填，用于后续登录）。
- **密码输入框**：输入密码（必填，至少6位字符）。
- **Register 按钮**：提交注册表单，成功后自动跳转到登录页面。若用户名已存在，会显示错误提示。
- **Back to Login 按钮**：返回登录页面。

#### 9.1.3 功能选择页面操作

- **执行模式单选按钮**：选择低信息密度模式，只显示今日任务。
- **学习模式单选按钮**：选择中信息密度模式，同时显示任务和笔记。
- **规划模式单选按钮**：选择高信息密度模式，显示全面的信息。
- **继续按钮**：提交选择，进入仪表板页面。
...
- **今日视图链接**：切换到单列表显示今日任务（截止日期为今天）。
- **时间段视图链接**：按上午/下午/晚上分组显示任务。
- **精力视图链接**：按高/中/低精力需求分组显示任务。

##### 9.2.2.3 任务搜索功能（规划模式）

- **关键字输入框**：输入搜索关键词（匹配标题和描述）。
- **开始日期选择器**：搜索起始日期。
- **结束日期选择器**：搜索结束日期。
- **搜索按钮**：执行搜索并显示结果。
...
- **归档按钮**：将目标标记为已完成，不再显示在活跃列表中。
- **删除按钮**：删除目标，弹出确认对话框选择是否同时删除关联任务。

##### 9.3.1.3 目标与任务树视图

- **目标与任务树按钮**（顶部栏）：显示/隐藏目标与任务的关系树状图。
- **树节点**：目标节点可展开显示关联任务，连线表示从属关系。
...
Lattice-Planner 作为个人任务管理工具，主要面向单用户或少量用户使用，数据规模相对有限（单个用户任务数通常在数百至数千条）。但在设计过程中，仍充分考虑了性能优化，以确保在数据增长时仍能保持良好的响应速度。以下从多个维度阐述性能优化策略：

#### 15.1.1 数据库性能优化

- **索引设计**：在任务表（memo）、笔记表（note）、目标表（goal）上建立了复合索引，包括 `user_id` + `deadline`、`user_id` + `created_at`、`user_id` + `status` 等常用查询字段。规划得分计算时，通过索引快速筛选用户数据，避免全表扫描。
- **查询优化**：得分计算采用一次性批量加载策略，将指定日期区间内的所有任务、笔记、目标数据一次性查询到内存，然后在内存中进行分组和计算，避免对数据库的频繁访问。
- **分页查询**：在任务列表展示时，支持分页加载，默认每页显示有限数量的任务，防止一次性加载过多数据导致内存压力。
- **连接池配置**：使用 HikariCP 连接池，配置合理的连接数上限（默认 10-20 个连接），避免数据库连接耗尽。

#### 15.1.2 应用层性能优化

- **缓存策略**：对用户偏好设置、思维模式选择等不经常变更的数据采用内存缓存（可集成 Redis）。规划得分结果可缓存 5-10 分钟，避免重复计算。
- **异步处理**：AI 总结生成采用异步执行，避免阻塞用户界面。桌面端 DDL 检查通过定时任务异步执行，不影响主线程响应。
- **懒加载**：目标-任务树视图采用懒加载策略，只在用户展开节点时加载子数据，减少初始加载时间。
- **事件驱动优化**：使用 Spring 的事件机制解耦业务逻辑，任务完成事件异步触发相关更新，提高并发处理能力。

#### 15.1.3 前端性能优化

- **静态资源压缩**：CSS 和 JS 文件采用 Gzip 压缩，减少传输大小。
- **按需加载**：不同思维模式对应的 JS 文件分开加载，避免一次性加载所有脚本。
- **AJAX 优化**：任务操作（如完成、搁置）采用 AJAX 异步提交，局部更新页面，提升用户体验。
- **客户端缓存**：浏览器本地存储用户偏好设置，减少服务器请求。

#### 15.1.4 监控与调优

- **性能监控**：集成 Spring Boot Actuator，提供健康检查、指标收集和 JVM 监控接口。
- **日志优化**：使用异步日志框架（如 Logback），避免日志写入阻塞业务处理。
- **内存管理**：定期清理过期会话和缓存，防止内存泄漏。
- **压力测试**：在开发阶段进行简单的负载测试，确保在 100 个并发用户下响应时间不超过 2 秒。

### 15.2 模块化与扩展性

Lattice-Planner 采用「核心层 + 插件层」的架构设计，通过事件驱动机制实现高度模块化和可扩展性。核心层负责基础的 CRUD 操作和数据管理，插件层通过监听事件实现功能扩展。这种设计使得新增功能时无需修改核心代码，降低了维护成本。

#### 15.2.1 核心架构扩展性

- **事件总线**：基于 Spring ApplicationEventPublisher 的事件机制，支持插件模块监听核心事件（如 TaskCreated、TaskCompleted），实现解耦扩展。
- **服务接口抽象**：核心服务（如 TaskService、GoalService）定义清晰的接口，新功能可以通过实现接口或继承现有服务扩展。
- **配置驱动**：通过 application.properties 支持功能开关，用户可通过配置启用/禁用特定模块。

#### 15.2.2 现有扩展模块示例

- **习惯打卡模块（Habit）**：
  - 监听每日统计事件，向规划得分加入「习惯维度」。
  - 实现方式：新增 Habit 实体和 HabitService，监听 TaskCompletedEvent 记录习惯完成情况。
- **番茄钟模块（Pomodoro）**：
  - 在任务执行时记录专注时间，并作为任务权重的附加参考。
  - 实现方式：新增 Pomodoro 实体，集成前端计时器，后端记录专注时长。

#### 15.2.3 未来扩展方向

基于当前架构和技术栈，以下是合适的扩展方向，按优先级和复杂度排序：

##### 15.2.3.1 短期扩展（低复杂度，易实现）

1. **多语言支持（i18n）**：
   
   - 扩展方向：添加多语言资源文件，支持中文、英文等界面语言切换。
   - 实现方式：使用 Spring MessageSource，在 Thymeleaf 模板中使用消息键。
   - 扩展价值：拓宽用户群体，适用于国际化使用。

2. **数据导出/导入功能**：
   
   - 扩展方向：支持任务、目标、笔记数据的 JSON/CSV 导出，及数据导入恢复。
   - 实现方式：新增 ExportController，提供下载接口；使用 Jackson 进行序列化。
   - 扩展价值：便于数据备份和迁移。

3. **提醒增强模块**：
   
   - 扩展方向：支持邮件、短信、微信等第三方提醒方式。
   - 实现方式：集成邮件服务（如 JavaMail）、短信 API，监听 DDL 事件发送提醒。
   - 扩展价值：提升任务提醒的及时性和多样性。
...
4. **团队协作功能**：
   ...
   - 扩展方向：支持多用户共享任务和目标，实现简单的团队任务管理。
   - 实现方式：扩展用户模型支持团队关系，新增共享权限控制；修改查询逻辑支持团队数据。
   - 扩展价值：从个人工具扩展到团队工具，增加商业价值。
   - 注意事项：需加强安全控制，防止数据泄露。

5. **移动端应用**：
   
   - 扩展方向：开发 React Native 或 Flutter 移动应用，复用后端 API。
   - 实现方式：保持现有 REST API 接口，前端重写为移动端界面。
   - 扩展价值：提升用户体验，支持随时随地使用。
   - 注意事项：移动端需优化网络请求和离线功能。

6. **高级统计分析**：
   
   - 扩展方向：增加更多维度的统计图表，如任务完成率趋势、精力消耗分析、目标达成预测。
   - 实现方式：扩展 InsightService，集成 Chart.js 或 D3.js 生成复杂图表。
   - 扩展价值：提供更深入的数据洞察，帮助用户优化规划。

##### 15.2.3.3 长期扩展（高复杂度）

7. **AI 增强功能**：
   
   - 扩展方向：集成更强大的 AI 模型，提供智能任务推荐、自动分类、风险预测。
   - 实现方式：接入 OpenAI API 或本地大模型，新增 AI 分析服务。
   - 扩展价值：提升智能化水平，从被动管理到主动辅助。
   - 注意事项：需考虑 API 成本和隐私保护。

8. **企业级功能**：
   
   - 扩展方向：支持企业级部署、多租户架构、权限管理系统、审计日志。
   - 实现方式：引入 Spring Security OAuth2、数据库分库分表、ELK 日志栈。
   - 扩展价值：支持企业级应用场景，增加商业化潜力。
   - 注意事项：架构复杂度大幅提升，需谨慎评估。

9. **第三方集成**：
   
   - 扩展方向：集成日历应用（如 Google Calendar）、项目管理工具（如 Trello）、时间跟踪工具。
   - 实现方式：实现 OAuth2 认证，新增同步服务。
   - 扩展价值：形成生态系统，提升用户粘性。

10. **云原生部署**：
    
    - 扩展方向：支持容器化部署、微服务拆分、Kubernetes 编排。
    - 实现方式：Docker 容器化，拆分为用户服务、任务服务、分析服务。
    - 扩展价值：提升可扩展性和可靠性，支持大规模部署。

### 15.3 扩展实施建议

- **渐进式扩展**：优先实现短期扩展，逐步积累用户反馈，再推进复杂功能。
- **测试驱动**：每个扩展模块需编写完整的单元测试和集成测试，确保不破坏现有功能。
- **文档同步**：扩展功能时同步更新用户文档和 API 文档。
- **性能监控**：扩展后进行性能回归测试，确保新增功能不影响整体性能。
- **用户反馈**：通过用户调研确定扩展优先级，避免开发无用功能。

通过以上性能优化和扩展设计，Lattice-Planner 不仅能满足当前需求，还具备良好的演进能力，为未来的功能扩展和性能提升奠定了坚实基础。
