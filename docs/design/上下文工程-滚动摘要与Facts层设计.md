...
> 状态：设计稿（实施见 `上下文工程P1-滚动摘要与Facts层实施记录.md`；P7 活体对照见 `Agent优化-P7窗口接线与证据空白.md`）
...
2. **facts 可能抽错**，且错误会被注入每一轮。三层防护降低概率，但消除不了。这是本设计最大的风险，也是建议「先只开摘要」的原因。摘要现已默认开；Facts 金标过门槛后仍关，见 P7。
