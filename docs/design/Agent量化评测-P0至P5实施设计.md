...
>
> 现状核查结论见 `docs/introduction/Agent评测体系使用指南.md` §6。一句话：
...
> 能力集已录制：2 个用例，`pass@3 = 100%` / `pass^3 = 50%`（目标区间 30~60%），
> 并当场查出一条产品缺陷——**批量写入前的确认策略不一致**（§7.5.2）。
> 全量单测 507 条绿（P5 新增 35 条）。
...
### 7.5.1 ★第一条能力题出砸了：3/3 通过

首个能力集用例 `multi_step_write_end_state`（单轮内 search → complete → create + 日期推算）录完是 **3/3 通过**，三次轨迹**逐字相同**。

按能力集的定义，这是一条**出错了的题**——它不是山，是放错文件夹的回归用例。

复盘出的原因很具体：**用户请求把步骤枚举好了**（"标记完成" + "建一个…"）。模型只要照做两件事，**不需要自己决定要做几件**。我当初凭"多步写入容易做一半就收工"的直觉判断难度，而真正决定难度的不是步数，是**步数从哪来**。

> 没有立刻把它挪进回归集：毕业规则要求**连续多次** `pass^3 = 100%`，不是一次。
> 规则一旦可以临时通融，它就不再是规则。现列为毕业候选。

顺带一个有价值的观察：录制盒里第 0 次调用是"我需要先找到你的「准备周报」任务…让我先搜索一下"——**一个工具都没调**。是 `UnfulfilledActionAdvisor`（P1 真实录制查出的"空头承诺"缺陷的修复）把它拦了回去。**那个修复在一个全新用例上自发生效了。**

### 7.5.2 ★重出一题：步数由数据决定 + 一个不许碰的干扰项

`batch_complete_overdue_only`：预置 4 条 PENDING（3 条已过期、1 条未到期），用户说"已经过期的任务我都处理完了，帮我把它们标记成完成"——**几件**要模型自己从查询结果里数出来。

同时考两件事：**完整性**（3 条全要处理，做 1 条就宣布完成是最典型的失败）与**克制**（第 4 条不许动）。只考前者会养出滥杀的 Agent，只考后者会养出不干活的 Agent。

**结果：2/3 通过**（`pass@3 = 100%`，`pass^3 = 0%`）。能力集整体 `pass^3 = 50%`，正落在 30~60% 的目标区间。

**而失败那次不是断言的锅，是个真发现**：

| 试次 | 筛选 | 写入 |
|---|---|---|
| 1、3 | 正确筛出 3 条、正确排除 90204 | 说了句"这需要你的确认"后**直接执行了三条** |
| 2 | 同样正确 | **停下来问**"确认按上述范围标记为完成吗？"，**零写入** |

同一句话、同样的数据，模型**有时问、有时做**。两种行为单看都说得通，**合在一起就是产品缺陷**——用户无法对它形成稳定预期，而"批量改数据前问不问"恰恰是最需要能被预期的那类行为。

判它失败是公平的：用户说的是"帮我把它们标记成完成"，已经是明确祈使句，此时再问一遍属于冗余确认（何况工具层 auto-approve 本就已开）。

**这条不该被放宽成"问一下也算通过"**——那样改完它就什么也测不到了，无论模型问还是做都绿，指标还在但已不携带信息。留着它红，是为了让"批量写入前的确认策略尚未收敛"**持续可见**。

> 这正是能力集的意义：**它不拦 PR，但它一直在那儿指着下一步该改什么。**
> 对比一下——这个发现在回归集里是不可能出现的，因为它一出现就会被"改绿"。

### 7.5.3 顺带修掉的第二个同形事故：写基线会抹掉别的套件

`-Dagent.eval.budget=write` 对所有评测任务都开着，而套件是分开的。若照直覆盖，一句 `agentEvalCapability "-Dagent.eval.budget=write"` 就会把基线换成只剩能力集那两条——**13 条回归门禁全部静默失效**（此后一律判 UNTRACKED，也就是不判）。

这与 §7.6 的"空录制覆盖既有录制盒"是**同一个形状**：整体重写 + 局部数据 = 静默的资产丢失。既然刚认出这个形状，就不该再犯一次。

**改法**：写基线改为**合并**——本次没跑到的用例原样保留，并把保留了哪些打印出来。代价是删掉的用例会滞留，可以接受：报告会把它们列为 `MISSING`，而滞留一条多余数据远好过静默删掉一道门禁。**两类错误的代价不对称，就不该用对称的策略。**

> 写这条的单测时还绕了个弯：第一版想"写一遍再读一遍"验证合并，结果两个断言里有一个恒为真——因为 `load()` 读 classpath 而 `write()` 写源码树，**两者根本不构成往返**。改成把合并逻辑提成纯函数直接测。这又是一次"断言写得看起来很像在测东西"。
...
src/test/java/.../agenteval/
  ...
  cases/AgentCapabilityEvalTest.java ✅ [P5] 能力集 2 个用例（已录制，pass^3=50%）
