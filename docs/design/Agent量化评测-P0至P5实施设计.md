...
> 库里有 k 条笔记就一定返回 k 条，哪怕后几条余弦为 0。而 CRAG 分级只看 `hits.get(0)`，
> 于是**低质片段一路畅通地进了上下文窗口**。代价是双份的：占预算，还给模型提供了编造的素材。
> 已写成断言 `noScoreFloor()`——加分数下限后它会变红，那时该更新的是断言而不是绕过它。
...
**为什么写成断言而不是直接修**：修法有好几种（提高 alpha、下调 lower、给纯向量场景单独标定、关键字缺席时走另一套阈值），选哪种得先量清楚代价。在那之前 `RagDegradeCalibrationTest` 是这个缺陷唯一的书面记录，而且它是活的——参数一改它就说话。修好后应当把断言**反过来**（`isEqualTo(0.0)`）而不是删掉。

同时补了一条 `correctDegradeRateHidesTheDefect()`：**正确降级率在缺陷存在时依然是 1.000**。
两个指标必须配着看，就像精确率之于召回率——只报正确降级率的话，这块缺陷完全看不见。
