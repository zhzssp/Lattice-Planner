 ...
 *
 * 在 MCP 场景下，后端直接做磁盘 IO，无需 Electron。
...
public class McpLocalFileService {
    ...
    public boolean isLocalTool(String name) {
        return Set.of("local.list_dir", "local.read_file", "local.read_pdf",
                "kb.ingest_local_doc", "kb.delete_local_doc").contains(name);
    }
    ...
    public List<Map<String, Object>> exportLocalTools() {
        ...
        tools.add(tool("local.read_file", "读取本地文本文件（utf-8），扩展名必须在白名单内",
                param("path", "绝对路径", "string", true)));
        tools.add(tool("local.read_pdf", "读取本地 PDF 文件的纯文本内容",
                param("path", "绝对路径", "string", true)));
        tools.add(tool("kb.ingest_local_doc", "把一份本地 md/txt/pdf 文档摄取进个人知识库",
                param("path", "绝对路径（受白名单约束）", "string", true)));
        tools.add(tool("kb.delete_local_doc", "按 path 反摄取本地文档：清空该路径在个人知识库中的所有 chunk",
                param("path", "要反摄取的绝对路径", "string", true)));
        return tools;
    }
    ...
    private Map<String, Object> readPdf(String pathStr) {
        ...
        // PDF 解析需要第三方库；这里用简单文本提取或标记未实现
        // 后续可集成 Apache PDFBox 或 iText
        try {
            // 先尝试直接读取（某些 PDF 有文本层可粗提取）
            byte[] bytes = Files.readAllBytes(path);
            String roughText = new String(bytes, StandardCharsets.UTF_8);
            // 简单过滤非文本字符
            String cleaned = roughText.replaceAll("[^\\x20-\\x7E\\u4e00-\\u9fff\\r\\n]", " ")
                    .replaceAll("\\s{3,}", "\n").trim();
            if (cleaned.length() < 50) {
                return errorResult("PDF 解析需要安装 PDFBox 依赖，当前仅支持粗提取。文本过少，请安装 Apache PDFBox。");
            }
            return Map.of("content", List.of(Map.of("type", "text",
                    "text", cleaned)), "isError", false);
        } catch (IOException e) {
            return errorResult("读取 PDF 失败：" + e.getMessage());
        }
    }
    ...
    private boolean isAllowedExtension(Path path) {
        ...
        return ALLOWED_EXTENSIONS.contains(ext) || ext.equals("pdf");
    }
