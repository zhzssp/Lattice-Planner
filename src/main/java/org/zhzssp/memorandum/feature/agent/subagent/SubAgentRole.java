...
public enum SubAgentRole {
    ...
    /** 规划专家：读素材 -> 拆解 -> 建目标/任务落库。需要 local/kb/planner/task/goal 领域。 */
    PLANNER(
            "规划专家",
            ...
            建议路径：必要时 local.read_file / local.read_pdf 读素材 ->
    ...
    RESEARCH(
            ...
            kb.list_backlinks / local.read_file 深入 -> 综合命中片段作答。
            引用某篇笔记用 [[标题]] 写法；最高 score < 0.4 时须明示"未找到强相关笔记"。
